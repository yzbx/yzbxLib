function Analyse_GlobalLightChange()
...
% img=imread([groundTruthPath,'\',filelist{frameNum+2}]);
    ...
    function show(roi,point)
        ...
        while frameNum<=roi(2)
            ...
            globalInput(~mask)=0;
            count=sum(mask(:));
            info(1)=sum(globalInput(:))/count;
            
            backgroundGlobalInput=input;
            mask=groundTruth<85;
            backgroundGlobalInput(~mask)=0;
            count=sum(mask(:));
            info(2)=sum(backgroundGlobalInput(:))/count;
